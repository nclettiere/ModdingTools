...
#define lwlog_debug(M, ...)                               \
  do                                                      \
  {                                                       \
    fprintf(stderr, GRAY "[DEBUG]   "                     \
                         "%s (%s:%d) " NONE M "\n",       \
            __func__, __FILE__, __LINE__, ##__VA_ARGS__); \
  } while (0)

#define lwlog_emerg_t(M, ...)                                                                          \
  do                                                                                                   \
  {                                                                                                    \
    fprintf(stderr, RED "[EMERG]   " NONE M YELLOW " errno: %s\n" NONE, ##__VA_ARGS__, clean_errno()); \
  } while (0)
#define lwlog_alert_t(M, ...)                                                                             \
  do                                                                                                      \
  {                                                                                                       \
    fprintf(stderr, PURPLE "[ALERT]   " NONE M YELLOW " errno: %s\n" NONE, ##__VA_ARGS__, clean_errno()); \
  } while (0)
#define lwlog_crit_t(M, ...)                                                                              \
  do                                                                                                      \
  {                                                                                                       \
    fprintf(stderr, YELLOW "[CRIT]    " NONE M YELLOW " errno: %s\n" NONE, ##__VA_ARGS__, clean_errno()); \
  } while (0)
#define lwlog_err_t(M, ...)                                                                              \
  do                                                                                                     \
  {                                                                                                      \
    fprintf(stderr, BROWN "[ERR]     " NONE M YELLOW " errno: %s\n" NONE, ##__VA_ARGS__, clean_errno()); \
  } while (0)
...
#define lwlog_notice_t(M, ...)                                                                          \
  do                                                                                                    \
  {                                                                                                     \
    fprintf(stderr, CYAN "[NOTICE]  " NONE M YELLOW " errno: %s\n" NONE, ##__VA_ARGS__, clean_errno()); \
  } while (0)
#define lwlog_info_t(M, ...)                                        \
  do                                                                \
  {                                                                 \
    fprintf(stderr, GREEN "[INFO]    " NONE M "\n", ##__VA_ARGS__); \
  } while (0)
#define lwlog_debug_t(M, ...)                                      \
  do                                                               \
  {                                                                \
    fprintf(stderr, GRAY "[DEBUG]   " NONE M "\n", ##__VA_ARGS__); \
  } while (0)
...
constexpr const char *BANNER = R"V0G0N(
 ...
 |  \/  |         | |   | (_)             |__   __|        | |    
 | \  / | ___   __| | __| |_ _ __   __ _     | | ___   ___ | |___ 
 | |\/| |/ _ \ / _` |/ _` | | '_ \ / _` |    | |/ _ \ / _ \| / __|
 | |  | | (_) | (_| | (_| | | | | | (_| |    | | (_) | (_) | \__ \
